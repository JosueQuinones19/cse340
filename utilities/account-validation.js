 ...
 * ********************************* */
// Aquí aplicamos la corrección: registrationRules (con 'r')
validate.registrationRules = () => {
  return [
    ...
    body("account_email")
      ...
      .notEmpty()
      .isEmail()
      .normalizeEmail() // El orden correcto: primero validar, luego normalizar
...
validate.checkLoginData = async (req, res, next) => {
  ...
  if (!errors.isEmpty()) {
    ...
    res.render("account/login", {
      ...
      title: "Login",
      nav,
      account_email, // Stickiness solo para el correo
    })
    return
  }
  next()
  ...
  return [
    ...
    body("account_email").trim().isEmail().normalizeEmail().withMessage("A valid email is required.")
      .custom(async (account_email, { req }) => {
        ...
        // Check if submitted email is different from the existing one
        if (account_email != account.account_email) {
          // If different, check if it already exists in the DB
          const emailExists = await accountModel.checkExistingEmail(account_email)
          if (emailExists) {
